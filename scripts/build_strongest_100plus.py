...
def main() -> int:
    ...
    # Load players.csv: player_id, theta
    ...
    for pid, theta in players:
        ...
        rows.append((pid, first, last, theta, stats["games"], last_game_str))
    ...
    with open(out_csv, "w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["rank", "player_id", "first_name", "last_name", "theta", "games", "last_game"])
        for i, (pid, first, last, theta, games, last_game) in enumerate(rows, 1):
            w.writerow([i, pid, first, last, round(theta, 6), games, last_game])
